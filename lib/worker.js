...
process.on('message', (message) => {
  ...
  switch (event) {
    ...
    case EVENT_RUN_BEGIN:
      console.log("Start reporting:", message.launch)
      reporter.runStart(message.launch);
      break;
    case EVENT_RUN_END:
      console.log("End reporting:", message)
      reporter
        .runEnd()
        .then(() => {
    ...
    case EVENT_TEST_BEGIN:
      let tags = [process.env.TEST_CASE_ID.split("/")].flat();
      let item = tags.filter(testCaseID => message.test.title.includes(testCaseID))
    ...
    case EVENT_TEST_END:
      let tagss = [process.env.TEST_CASE_ID.split("/")].flat();
      let items = tagss.filter(testCaseID => message.test.title.includes(testCaseID))
